...
ModelGenerator.prototype.parseTypo = function(typoPart){
	return this.parseModelOptions(typoPart);
};
...
ModelGenerator.prototype.parseModelOptions = function(typo){
	...
	cacheTypo = this.matchSequelizeType(cacheTypo);
...
ModelGenerator.prototype.matchSequelizeType = function(type){
	var sequelize = require("sequelize");

	var commonType = Object.keys(sequelize).filter(function(key){
		return key === type.toUpperCase();
	});

	var sequelizeType = commonType.shift();
	if(sequelizeType){
		return TYPE_DESC + sequelizeType;
	}else{
		console.log("Ornito Info: ", "Associations is not supported yet:", type);
		return TYPE_DESC + "TODO";
	}
};
